...
class SILogLoss(nn.Module):
    ...
    def forward(self, input, target, mask=None, interpolate=True):
        ...
        # Clipping the input and target values to prevent zero values
        input = input.clamp(self.epsilon, float('inf'))
        target = target.clamp(self.epsilon, float('inf'))

        g = torch.log(input) - torch.log(target)

        # Calculating the variance and mean of the difference
        # between the predicted and ground truth depth values
        Dg = torch.var(g) + 0.15 * torch.pow(torch.mean(g), 2)

        # Scaling the loss by a factor of 10 and taking the square root
