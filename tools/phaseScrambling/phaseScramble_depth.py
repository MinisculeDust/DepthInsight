...
def phaseScramble_depth_replacePath(rgb_path, depth_path, originalKeyword='', savingKeyword=''):
    ...
    rgb_saved_path = rgb_path.replace(originalKeyword, savingKeyword)
    depth_saved_path = depth_path.replace(originalKeyword, savingKeyword)
    directory = os.path.dirname(rgb_saved_path)
    if not os.path.exists(directory):
        os.makedirs(directory)
    directory = os.path.dirname(depth_saved_path)
    if not os.path.exists(directory):
        os.makedirs(directory)
    # rgb_saved_path = saving_root + 'nyu_images/' + rgb_path.split('/')[-1].split('.')[0] + '.jpg'
    # depth_saved_path = saving_root + 'nyu_depths/' +  depth_path.split('/')[-1].split('.')[0] + '.png'
    ...
    random_matrix_saved_path = depth_path.replace(originalKeyword, savingKeyword)
    random_matrix_saved_path = directory + '/' + random_matrix_saved_path.split('/')[-1].split('.')[0] + '.npy'
    np.save(random_matrix_saved_path, RandomPhase)
    # random_matrix_saved_path_json = directory + '/' + random_matrix_saved_path.split('/')[-1].split('.')[0] + '.json'
    # with open(random_matrix_saved_path_json, 'w') as f:
    #     json.dump(RandomPhase.tolist(), f)


def phaseScramble_saturation_depth_replacePath(rgb_path, depth_path, originalKeyword='', savingKeyword=''):
